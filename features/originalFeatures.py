...
def diff_words_title_keywords():
	...
	for i in range(0, len(data)):
		if(data[i]['targetKeywords']):	
			diff_words_title_keywords_vector.append(abs(word_count_function(data[i]['postText'][0]) - keyword_count(data[i]['targetKeywords'])));
		else:
...
def diff_chars_title_keywords():
	...
	for i in range(0, len(data)):
		if(data[i]['targetKeywords']):	
			diff_chars_title_keywords_vector.append(abs(character_count_function(data[i]['postText'][0]) - character_count_function(data[i]['targetKeywords'])));
		else:
...
def ratio_words_descr_title():
	...
	for i in range(0, len(data)):
		if(data[i]['targetDescription'] and data[i]['postText']):	
			ratio_words_descr_title_vector.append(word_count_function(data[i]['targetDescription'])/word_count_function(data[i]['postText'][0]))
		else:
...
def ratio_paragraphs_title():
	...
	for i in range(0,len(data)):
		if(data[i]['targetParagraphs'] and data[i]['postText'][0]):
			ratio_paragraphs_title_vector.append(character_count_function(data[i]['targetParagraphs'])/character_count_function(data[i]['postText'][0]));
		else:
...
def ratio_description_title():
	...
	for i in range(0,len(data)):
		if(data[i]['postText'][0] and str(data[i]['targetDescription'])):
			ratio_description_title_vector.append(character_count_function(data[i]['targetDescription'])/character_count_function(data[i]['postText'][0]));
		else:
...
def ratio_paragraphs_description():
	...
	for i in range(0,len(data)):
		if(data[i]['targetParagraphs'] and data[i]['targetDescription']):
			ratio_paragraphs_description_vector.append(character_count_function(data[i]['targetParagraphs'])/character_count_function(data[i]['targetDescription']));
		else:
...
def ratio_article_title_post_title():
	...
	for i in range(0,len(data)):
		if(data[i]['targetTitle'] and data[i]['postText'][0]):
			ratio_article_title_post_title_vector.append(character_count_function(data[i]['targetTitle'])/character_count_function(data[i]['postText'][0]));
		else:
...
def ratio_words_article_title_post_title():
	...
	for i in range(0,len(data)):
		if(data[i]['targetTitle'] and data[i]['postText'][0]):
			ratio_words_article_title_post_title_vector.append(word_count_function(data[i]['targetTitle'])/word_count_function(data[i]['postText'][0]));
		else:
...
def ratio_words_image_title():
	...
	all_files_with_path = glob.glob("D:/msc/q3/information retrieval/papers to review/nlp project/train4/custom/media/*.jpg") + glob.glob("D:/msc/q3/information retrieval/papers to review/nlp project/train4/custom/media/*.png")
...
def diff_chars_title_image():
	...
	#make ratio
	for i in range(0,len(data)):
		diff_chars_title_image_vector.append(abs(all_text_image_no[i]-character_count_function(data[i]['postText'][0])));
...
def ratio_chars_image_title():
	...
	for i in range(0,len(data)):
		if(data[i]['postMedia'] and data[i]['postText'][0]):
			ratio_chars_image_title_vector.append(all_text_image_no[i]/character_count_function(data[i]['postText'][0]));
		else:
...
df = DataFrame(original_features, columns=['id', 'PostTitleWordsNumber', 'PostTitleCharsNumber', 'DiffNoWordsPostTitleAndKeywords', 'DiffNoCharsPostTitleAndKeywords', 'RatioNoWordsArticleDescriptionAndPostTitle', 'QuestionMarksNoPostTitle', 'RatioNoCharArticleParagraphPostTitle', 'RatioNoCharArticleDescriptionPostTitle', 'RatioNoCharArticleTitlePostTitle', 'RatioNoWordsArticleTitlePostTitle', 'RatioNoWordsPostImagePostTitle', 'DiffCharsPostTitlePostImage', 'RatioCharPostImagePostTitle', 'FormalWordsPostTitle', 'RatioNoCharArticleParagraphArticleDescription'])
print(df)
export_csv = df.to_csv (r'D:\msc\q3\information retrieval\papers to review\nlp project\train4\custom\originalFeaturesTrainMihai.csv', index = None, header=True) 
