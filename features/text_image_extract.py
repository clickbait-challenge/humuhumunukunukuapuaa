...
for i in range(0, len(all_files_with_path)):
	if(pytesseract.image_to_string(Image.open(all_files_with_path[i]))):
		image_text_no_words.append(word_count_function(pytesseract.image_to_string(Image.open(all_files_with_path[i]))));
		image_text_no_chars.append(character_count_function(pytesseract.image_to_string(Image.open(all_files_with_path[i]))));
	else:
